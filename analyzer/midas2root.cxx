...
public:
  ...
  void BeginRun(int transition,int run,int time){
    ...
    for(int i=0; i<8; i++) waveforms[i] = new TH1D(Form("waveform%d",i),Form("waveform%d",i),1024,0,1024);	  
    for(int i=0; i<16; i++) waveforms[i] = NULL;
    ...
    for(int i=0; i<8; i++) fTree1->Branch(Form("Channel%d",i),"TH1D",&(waveforms[i]));

    fTree2 = new TTree("midas_data2","Second Digitizer");

    fTree2->Branch("midasEvent",&midasEvent2,"midasEvent/I");
    fTree2->Branch("timestamp",&timestamp,"timestamp/I");
    fTree2->Branch("serialnumber",&serialnumber,"serialnumber/I");
    fTree2->Branch("freqsetting",&frequency,"freqsetting/I");
    fTree2->Branch("triggerTime",&digiCounter2,"triggerTime/g");
    std::cout << "Set Branches for histograms" << std::endl;
    for(int i=0; i<8; i++) fTree2->Branch(Form("Channel%d",i),"TH1D",&(waveforms[i+8]));

#ifdef USE_V792    
    
    fTree1->Branch("nchannels",&nchannels,"nchannels/I");
    fTree1->Branch("adc_value",adc_value,"adc_value[nchannels]/I");
#endif
  ...
  bool ProcessMidasEvent(TDataContainer& dataContainer){
    ...
    if(dt743){
      ...
      for(int i = 0; i < measurements.size(); i++){
        ...
        //if(nsamples!=1024) std::cout << "Number of samples isn't 1024" << std::endl;
  ...
  std::string SetFullOutputFileName(int run, std::string midasFilename)
  {
    ...
    if (part == 2) {
      ...
      sprintf(buff,"/home/wcte/data_root/root_run_%.6d_%.4d.root", run, num[1]);
      printf("Using filename %s\n",buff);
    } else {
      sprintf(buff,"/home/wcte/data_root/root_run_%.6d.root", run);
    }
